...
import Filters, {emptyFilter, filterRestaurants, FilterState} from "@/components/Filters";
        ...
        <main className="min-h-screen bg-slate-950 text-white">
            <div className="mx-auto max-w-2xl px-5 py-10">
                <header className="mb-8 flex items-center justify-between gap-2">
                  ...
                  <span
                      className="inline-block h-2 w-2 animate-spin rounded-full border border-white/40 border-t-white"/>
                  동기화 중
                </span>
                            ) : justSynced ? (
                                "최신!"
                ...
                {sync === "error" && (
                    <div
                        className="mb-6 rounded-lg border border-amber-500/30 bg-amber-500/10 p-3 text-xs text-amber-200">
...
function WinnerCard({winner}: { winner: Restaurant }) {
    ...
    return (
        <div
            className="mt-6 rounded-2xl bg-gradient-to-br from-amber-400/20 to-pink-500/20 p-5 ring-1 ring-amber-400/40">

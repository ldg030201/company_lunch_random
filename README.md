...
Next.js 16부터 `/_next/*` 자산에 기본적으로 cross-origin 차단이 있어서 [next.config.ts](next.config.ts)의 `allowedDevOrigins`에 사설 IP 대역(`192.168.0.*`, `10.*.*.*` 등)을 미리 넣어뒀습니다. 필요하면 추가하세요.
...
### 제약
서버가 **단일 Node 프로세스**라는 전제로 동작합니다. Vercel Edge/Serverless 같은 환경에서는 매 요청이 독립 메모리라 broker가 제 기능을 못 해요. 로컬 개발 + 자체 호스팅(Railway/Render/Docker 등) 상시 가동 서버에 적합.
